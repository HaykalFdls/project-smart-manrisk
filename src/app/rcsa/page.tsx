...
const getLevelFromBesaran = (besaran: number | null | undefined) => {
  if (besaran === null || besaran === undefined) return { label: '-', className: 'bg-muted/40 text-muted-foreground' };
...
const CalculatedValue = ({ label, value, className }: { label: string; value: string | number | null; className?: string }) => (
  <div className="space-y-1">
    ...
    <div className={`flex h-10 w-full items-center justify-center rounded-lg border border-input bg-muted/50 px-3 py-2 text-sm font-bold shadow-sm ${className}`}>
      {value || '-'}
    </div>
  </div>
);
...
const RiskCard = memo(({ row, index, onChange }: RiskCardProps) => {
  ...
  return (
    ...
    >
      <Card key={row.id ?? index} className="shadow-md hover:shadow-lg transition-shadow duration-300 border rounded-2xl">
  ...
  return (
    <div className="flex flex-1 flex-col p-4 md:p-6 lg:p-8 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
        ...
        <div className="flex items-center gap-2">
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <Button disabled={data.length === 0} className="bg-blue-600 hover:bg-blue-700 text-white">
      ...
      <Alert className="mb-6 border-l-4 border-blue-600 bg-blue-50">
        ...
        <AlertDescription>
          Daftar potensi risiko dibawah mohon untuk segera di isi dikarenakan ada batasan waktu pengumpulan.
        </AlertDescription>
      </Alert>
